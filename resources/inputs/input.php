...
class input extends \resources\_ {
	...
	function render($data,$adminRender){
		...
		if ($adminRender){
			...
			$module = $data['module'];
			$inputO = \resources\inputs\input::getInputClass($module);
	...
	static function getInputsList($inputtypes=false){
		$return = array();
		...
		if (is_array($inputtypes)){
			...
			foreach (glob("./resources/inputs/*/input.php") as $input) {
				$t = explode("/",$input);
	...
	static function getInputClass($input){
		...
		return "\\resources\\inputs\\{$input}\\input";
