...
class timer {
	...
	function _stop($namespace,$class,$method,$args){

		return $this->stop(array("$namespace" => array("Class" => $class, "Method" => $method)), $args, true);
	}
	function stop($msg = "", $arguments = "", $from_stop=false) {
		...
		$this->totalTime($msg, $arguments, $from_stop);
		if ($this->endTimer && $this->startTimer) {
			return $this->endTimer - $this->startTimer;
		} else return "0";
	}

	private function totalTime($msg = "", $arguments = "", $force_stop=false) {
  ...
  'args' => */
				...
				if (isset($trace[2]["args"])) array_walk_recursive($trace[2]["args"], "filter");
				if (isset($trace[3]["args"])) array_walk_recursive($trace[3]["args"], "filter");
			//	if (isset($trace[3]["args"]) && $trace[3]["args"]) $trace[3]["args"] = htmlspecialchars($trace[3]["args"]);
